...
def store_requirements_in_vector_db(requirements, code, language="COBOL"):
    """
    ...
    # Generate unique IDs using timestamp and UUID to avoid conflicts
    base_id = int(time.time() * 1000)  # millisecond timestamp
    
    points = [
        PointStruct(
            id=base_id + i,  # Unique ID for each point
            vector=embeddings[i].tolist(),
            payload={
                "requirement": texts[i],
                "source_code": code,  # Store full source code
                "timestamp": time.time(),
                "language": language  # Add metadata for future filtering
            }
        )
...
def agent_extract(code: str, language: str = "COBOL"):
    ...
    store_requirements_in_vector_db(requirements_text, code, language)
    return requirements
